    ...
    <div className="min-h-screen grid place-items-center px-4">
      <div className="w-full max-w-md p-6 rounded-xl bg-white dark:bg-gray-800 shadow">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button className="w-full px-4 py-2 rounded-md bg-primary text-white">Create account</button>
        </form>
        <div className="mt-3 text-sm">
          <Link to="/auth/login" className="text-primary">Already have an account? Sign in</Link>
        </div>
      </div>
    </div>
  )
